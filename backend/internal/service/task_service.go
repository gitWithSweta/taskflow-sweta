...
func NewTaskService(projects projectRepository, tasks taskRepository, users userRepository) *TaskService {
	return &TaskService{projects: projects, tasks: tasks, users: users}
}

func (s *TaskService) List(ctx context.Context, uid, projectID uuid.UUID, status *string, assignee *uuid.UUID, limit, offset int) ([]model.Task, int, error) {
...
func (s *TaskService) Create(ctx context.Context, uid, projectID uuid.UUID, title string, description *string, status, priority string, assignee *uuid.UUID, dueDate *time.Time) (*model.Task, error) {
	okAccess, err := s.projects.UserHasAccess(ctx, uid, projectID)
	if err != nil {
		return nil, err
	}
	...
	if dueDate != nil && dueDateBeforeTodayUTC(*dueDate) {
		return nil, &errs.ValidationError{Fields: map[string]string{"due_date": "cannot be in the past"}}
	}
	return s.tasks.Create(ctx, title, description, status, priority, projectID, uid, assignee, dueDate)
}

func (s *TaskService) Patch(ctx context.Context, uid, taskID uuid.UUID, patch model.TaskPatch) (*model.Task, error) {
...
func (s *TaskService) Delete(ctx context.Context, uid, taskID uuid.UUID) error {
	cur, err := s.tasks.GetByID(ctx, taskID)
	if err != nil {
		return err
	}
